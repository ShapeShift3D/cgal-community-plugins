...
int stkCGALPolyLineSetToPolygonSet::RequestData(vtkInformation *,
...
//---------------------------------------------------
bool stkCGALPolyLineSetToPolygonSet::ProcessPwh(vtkPolyData* pwhPoly, int firstCoord, int secondCoord)
{
	vtkNew<vtkPolyDataConnectivityFilter> connectivityFilter;
	...
	for (vtkIdType i = 0; i < nbOfPolylines; ++i)
	{
		...
		Polygon_2 polygon;
		stkCGALPolygonUtilities::vtkPolyDataToPolygon2(cleanFilter->GetOutput(), polygon, firstCoord, secondCoord);

		CGAL::Orientation orient = polygon.orientation();
		if (this->DebugMode)
			stkCGALPolygonUtilities::PrintPolygonProperties(polygon, "Polyline " + std::to_string(i), false);
