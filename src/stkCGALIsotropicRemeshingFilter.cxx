...
//-----------------------------------------------------------------------------
int stkCGALIsotropicRemeshingFilter::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  if (std::strcmp(this->MeshingMaskArrayName.c_str(), "None") != 0)
  {
    masking = true;
    ...
    threshold->SetInputArrayToProcess(
      0, 0, 0, vtkDataObject::FIELD_ASSOCIATION_CELLS, this->MeshingMaskArrayName.c_str());
  ...
  if (!masking && this->ProtectConstraints)
  {
    ...
    PMP::border_halfedges(
      faces(mesh), mesh, boost::make_function_output_iterator(halfedge2edge(mesh, border)));
...
//-----------------------------------------------------------------------------
void stkCGALIsotropicRemeshingFilter::PrintSelf(std::ostream& os, vtkIndent indent)
{
 ...
 */
int stkCGALIsotropicRemeshingFilter::RequestInformation(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  vtkInformation* inInfo = inputVector[0]->GetInformationObject(0);
  vtkInformation* outInfo = outputVector->GetInformationObject(0);
