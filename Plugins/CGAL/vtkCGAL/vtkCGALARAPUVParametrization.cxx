/**
* \class vtkCGALARAPUVParametrization
*
...
//---------VTK----------------------------------
#include "vtkCGALARAPUVParametrization.h"
...
vtkStandardNewMacro(vtkCGALARAPUVParametrization);

// -----------------------------------------------------------------------------
vtkCGALARAPUVParametrization::vtkCGALARAPUVParametrization()
{
	this->Lambda = 1000;
...
//---------------------------------------------------
int vtkCGALARAPUVParametrization::RequestData(vtkInformation*,
												...
												vtkInformationVector* outputVector)
{
	...
	if (!vtkCGALUtilities::vtkPolyDataToPolygonMesh(inputMesh, surfaceMesh))
	{
	...
	typedef SMP::ARAP_parameterizer_3<Surface_Mesh, Border_Parameterizer> Parameterizer;

	SMP::Error_code err = SMP::parameterize(surfaceMesh,
		Parameterizer(
		Border_Parameterizer(),
			CGAL::Eigen_solver_traits<Eigen::BiCGSTAB<CGAL::Eigen_sparse_matrix<double>::EigenType>>(),
			this->Lambda,
			(unsigned int)this->MaximumNumberOfIterations,
			this->Tolerance),
		bhd,
		uv_map);
...
bool vtkCGALARAPUVParametrization::UpdatePointCoordinates(SurfaceMesh& sm, VertexUVMap& uv_map, vtkPolyData* polyData)
{
